    ...
    *,
    ...
    def _values_for_player(pname):
        ...
        for m, col in zip(metrics_sel, colnames):
            ...
            v = None
            try:
                if "player" in getattr(df_all, "columns", []):
        ...
        return _np.array(row_vals, dtype=float)
    ...
    if include_bars and len(metrics_sel) > 0:
        ...
        for r in range(start, end):
            for c in range(0, 12, int(12/cols_per_row)):
                ...
                va   = _val_or_nan(values_a, idx) if values_a is not None else _np.nan
                vb   = _val_or_nan(values_b, idx) if values_b is not None else _np.nan

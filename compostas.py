...
# ===================== SIDEBAR — Controls =====================
st.sidebar.header("⚙️ Settings")
up = st.sidebar.file_uploader(\1, key='upload_xlsx_main')", type=["xlsx"])
TOPN = st.sidebar.slider(\1, key='slider_main')
pos_filter = st.sidebar.text_input(\1, key='textinput_main')", value="")
team_filter = st.sidebar.text_input("Filter by Team (exact match)", value="")
demo_mode = st.sidebar.checkbox(\1, key='checkbox_main')", value=False)
min_minutes = st.sidebar.number_input(\1, key='numberinput_main')

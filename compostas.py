...
def _fix_npxg_block(df):
    ...
    if "npxG" in df.columns and "Minutes played" in df.columns:
        with np.errstate(divide="ignore", invalid="ignore"):
            mp = pd.to_numeric(df["Minutes played"], errors="coerce").replace(0, np.nan).astype(float)
            df["npxG per 90"] = (pd.to_numeric(df["npxG"], errors="coerce").astype(float) / mp) * 90.0
    # npxG por finalização
    if "npxG" in df.columns and "Shots" in df.columns:
        with np.errstate(divide="ignore", invalid="ignore"):
            df["npxG per Shot"] = pd.to_numeric(df["npxG"], errors="coerce").astype(float) / pd.to_numeric(df["Shots"], errors="coerce").replace(0, np.nan).astype(float)
    # G-xG
...
def make_player_report_pdf(df, player_a, player_b, metrics, color_a="#2A9D8F", color_b="#E76F51",
    ...
    def _metric_rank_info(df, metric, player_name):
        ...
        try:
            rk = int(order.loc[df["Player"] == player_name].iloc[0])
        except Exception:
            rk = None
        ...
        if tot > 0:
            ...
            if not np.isfinite(rng) or rng == 0:
                ...
                try:
                    val = float(s.loc[df["Player"] == player_name].iloc[0])
                    norm = (val - smin) / (rng + 1e-9)
...
# ---------- Professional A4 PDF Report (guarded) ----------
try:
    _ = df_all; _ = p1; _ = p2; _ = metrics_sel; _ = color_a; _ = color_b
    import streamlit as st

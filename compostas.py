...
if p1 and metrics_sel:
    ...
    try:
        pdf_buf = make_radar_bars_pdf_a4_pro(df_all, p1, None if p2 == "—" else p2, metrics_sel, color_a, color_b, player_photo_bytes=player_photo_bytes, crest_bytes=crest_bytes)
    except TypeError:
        pdf_buf = make_radar_bars_pdf_a4_pro(df_all, p1, None if p2 == "—" else p2, metrics_sel, color_a, color_b, player_photo_bytes=player_photo_bytes, crest_bytes=crest_bytes)
st.download_button("⬇️ 
...
Download Radar + Barras (PDF A4)", data=pdf_buf.getvalue(),

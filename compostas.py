...
def make_radar_bars_pdf_a4_pro(df: pd.DataFrame, player_a: str, player_b: str | None, metrics: list[str],
    ...
    for i, m in enumerate(metrics[:cols_per_row * total_rows]):
        ...
        if shared_ax is None:
            ...
            ax.set_xlim(0, 1)
            ax.set_xticks([0, 0.5, 1])
            ax.set_xticklabels(["0%","50%","100%"], fontsize=7 if r == (total_rows - 1) else 0)
        else:
            ax = fig.add_subplot(sub_gs[r, c], sharex=shared_ax)
            ...
            def _make_bars_page(chunk, title_suffix="(cont.)"):
                ...
                for i, m in enumerate(chunk):
                    ...
                    if shared_ax2 is None:
                        ...
                        axb.set_xticklabels(["0%","50%","100%"], fontsize=7 if r == (per_rows - 1) else 0)
                    else:
                        axb = fig2.add_subplot(sub[r, c], sharex=shared_ax2)

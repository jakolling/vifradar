...
OFFENSIVE_COMPONENTS = [
    "Successful attacking actions per 90","xG per 90","xA per 90","Key passes per 90","Deep completions per 90",
    "Deep completed crosses per 90","Progressive runs per 90","Passes to penalty area per 90","Smart passes per 90",
    "Crosses to goalie box per 90","Touches in box per 90",
]
DEFAULT_OFF_WEIGHTS = {
    "Successful attacking actions per 90": 1.0,"xG per 90": 1.0,"xA per 90": 1.0,"Key passes per 90": 1.0,
    "Deep completions per 90": 1.0,"Deep completed crosses per 90": 0.8,"Progressive runs per 90": 0.6,
    "Passes to penalty area per 90": 0.8,"Smart passes per 90": 0.5,"Crosses to goalie box per 90": 0.4,
    "Touches in box per 90": 0.4,
}
PHYS_COLS = {
    "distance_total_m": "Distance P90","running_m": "Running Distance P90","hi_m": "HI Distance P90",
    "hsr_m": "HSR Distance P90","sprint_m": "Sprint Distance P90","hsr_cnt": "HSR Count P90",
    "sprint_cnt": "Sprint Count P90","expl_hsr_cnt": "Explosive Acceleration to HSR Count P90",
...
DEF_COLS = [
    ...
    "Aerial duels per 90","Aerial duels won, %","PAdj Sliding tackles","PAdj Interceptions","Shots blocked per 90",
]
GK_METRICS = [
...
PRESETS = {
    "forward": ["npxG per 90","xG per 90","Shots per 90","Shots on target, %","xA per 90","Key passes per 90",
                "Touches in box per 90","Progressive runs per 90","Deep completions per 90","Finishing","Poaching",
                "Aerial Threat","Work Rate Offensive","Offensive Intensity","Offensive Explosion"],
    "winger": ["Dribbles per 90","Dribbles won, %","Crosses per 90","Accurate crosses, %","Deep completed crosses per 90",
               "Progressive runs per 90","xA per 90","Key passes per 90","Creativity","Progression",
               "Work Rate Offensive","Offensive Intensity","Offensive Explosion"],
    "attacking_midfielder": ["Creativity","Progression","xA per 90","Key passes per 90","Smart passes per 90",
                             "Deep completions per 90","xG Buildup","Work Rate Offensive","Offensive Intensity","Offensive Explosion"],
    "central_midfielder": ["Progression","Passing Quality","Creativity","xG Buildup","PAdj Interceptions",
                           "Successful defensive actions per 90","Work Rate Defensive","Defensive Intensity"],
    "defensive_midfielder": ["Defence","Progression","Passing Quality","Discipline","Involvement","xG Buildup",
                             "PAdj Interceptions","Successful defensive actions per 90",
                             "Work Rate Defensive","Defensive Intensity","Defensive Explosion"],
    "full_back": ["Progression","Creativity","Passing Quality","Defence","Aerial Defence","Deep completed crosses per 90",
                  "Progressive runs per 90","Crosses per 90","Work Rate Defensive","Defensive Intensity","Defensive Explosion"],
    "center_back": ["Defence","Aerial Defence","Aerial duels per 90","Aerial duels won, %","Defensive duels per 90",
                    "Defensive duels won, %","PAdj Interceptions","PAdj Sliding tackles","Passes to final third per 90",
                    "Accurate passes %","Work Rate Defensive","Defensive Intensity","Defensive Explosion"],
    "goalkeeper": GK_METRICS + ["Passes per 90","Accurate passes, %","Long passes per 90","Accurate long passes, %",
                                "Aerial duels per 90","Aerial duels won, %"],
    "general_summary": ["Involvement","Box Threat","Creativity","xG Buildup","Progression","Defence","Discipline","Passing Quality"],
    "defensive_actions": ["Defence","Aerial Defence","PAdj Interceptions","Successful defensive actions per 90",
                          "Defensive duels won, %","Shots blocked per 90","Discipline"],
    "playmaking": ["Creativity","Passing Quality","Progression","xG Buildup","Successful attacking actions per 90",
                   "Deep completions per 90","Involvement","Key passes per 90"],
...
NEGATE_METRICS = {
    "Conceded goals per 90": True,"xG against per 90": True,"Fouls per 90": True,"Yellow cards per 90": True,"Red cards per 90": True,
}
...
def compute_derived_metrics(df: pd.DataFrame) -> pd.DataFrame:
    df = df.copy()
    needed = set([
        "xG","Penalties taken","Goals","Shots","Touches in box per 90","xA per 90","Shot assists per 90","Key passes per 90",
        "Deep completions per 90","Deep completed crosses per 90","Accurate passes %","Accurate passes, %",
        "Smart passes per 90","Through passes per 90","Passes to penalty area per 90","Accurate smart passes, %",
        "Accurate through passes, %","Accurate passes to penalty area, %","Progressive passes per 90","Progressive runs per 90",
        "Dribbles per 90","Accelerations per 90","Accurate progressive passes, %","Successful dribbles, %",
        ...
        "Back passes per 90","Accurate back passes, %","Passes per 90","Head goals per 90","Goal conversion, %",
        "Received passes per 90","Fouls per 90","Yellow cards per 90","Red cards per 90","Minutes played"
    ])
    ...
    for col, w in xgb_w.items():
        if col in df.columns:
            s = df[col].fillna(0)
            if "%" in col and s.max() > 1: s = s / 100.0
            parts.append(_zscore(s) * w); total_w += w
    if parts and total_w > 0: df["xG Buildup"] = sum(parts) / total_w
    ...
    for c, w in def_w.items():
        if c in df.columns:
            ...
            parts.append(_zscore(s) * w); total_w += w
    if parts and total_w > 0: df["Defence"] = sum(parts) / total_w

    inv_w = {
        "Passes per 90": .20, "Received passes per 90": .15, "Touches per 90": .05, "Defensive duels per 90": .10,
        "PAdj Interceptions": .10, "Successful defensive actions per 90": .05, "Touches in box per 90": .10,
        "Offensive duels per 90": .10, "Progressive runs per 90": .05, "Aerial duels per 90": .10,
    }
    parts = []
    ...
    poach_w = {"npxG per Shot": .35, "Goal conversion, %": .30, "Touches in box per 90": -.25, "Received passes per 90": -.20, "Non-penalty goals per 90": .40}
    parts = []
    ...
    if parts and total_w > 0: df["Passing Quality"] = sum(parts) / total_w
...
def compute_defensive_production(df: pd.DataFrame) -> pd.Series:
    ...
    parts = [
        _safe_s(df, "Successful defensive actions per 90"), def_eff, aer_eff,
        _safe_s(df, "PAdj Sliding tackles"), _safe_s(df, "PAdj Interceptions"), _safe_s(df, "Shots blocked per 90"),
    ]
    total = parts[0]
    for p in parts[1:]: total = total + p
    return total
...
def plot_radar(df: pd.DataFrame, player_a: str, player_b: str | None, metrics: list[str],
    ...
    try: radar.spoke(ax=ax, color="#c9c9c9", linestyle="--", alpha=0.18)
    except Exception: pass
...
def render_metric_rank_bars(dfin: pd.DataFrame, player_a: str, metrics: list[str], player_b: str | None = None):
    ...
    if player_b: _render_for(player_b, "Jogador B")
...
def make_radar_bars_png(df: pd.DataFrame, player_a: str, player_b: str | None, metrics: list[str],
    ...
    try: radar.spoke(ax=ax_radar, color="#c9c9c9", linestyle="--", alpha=0.18)
    except Exception: pass
    ...
    if player_b: _draw_bar_block(start_row=rows_per_player, player_name=player_b)
    buf = io.BytesIO()
    fig.tight_layout()
...
def make_radar_bars_pdf_a4(df: pd.DataFrame, player_a: str, player_b: str | None, metrics: list[str],
    ...
    """
    ÚNICA função de geração de PDF A4 (consolida versões antigas/PRO).
    - Header opcional com foto (esq.) e escudo (dir.)
    - Radar amplo
    - Barras compactas em grid
    """
    ...
    if player_photo_bytes:
        try:
            img = Image.open(io.BytesIO(player_photo_bytes))
            ax_photo.imshow(img); ax_photo.set_xticks([]); ax_photo.set_yticks([])
        except Exception: pass
    if crest_bytes:
        try:
            img = Image.open(io.BytesIO(crest_bytes))
            ax_crest.imshow(img); ax_crest.set_xticks([]); ax_crest.set_yticks([])
        except Exception: pass
    ...
    sub_gs = GridSpecFromSubplotSpec(nrows=total_rows, ncols=cols_per_row, subplot_spec=gs_page[9:12, 0:12], wspace=0.16, hspace=0.12)

    def _draw_bar(ax, m, player_name, row_idx):
        info = _metric_rank_info(df, m, player_name)
        ...
        if row_idx == (total_rows - 1):
            ax.set_xticks([0, 0.5, 1]); ax.set_xticklabels(["0%","50%","100%"], fontsize=7); ax.tick_params(axis="x", pad=0)
        else:
    ...
    for i, m in enumerate(metrics[:cols_per_row * total_rows]):
        r = i // cols_per_row; c = i % cols_per_row
        ax = fig.add_subplot(sub_gs[r, c])
        _draw_bar(ax, m, player_a, r)
...
min_minutes = st.sidebar.number_input(
    "Minimum minutes played", min_value=0, value=0, step=90,
...
Weighted sum of attacking contributions such as xG/xA, key passes, deep completions, progressive runs, etc.
    """)
    st.header("Defensive Metrics")
    st.markdown("""
**Defensive Production (per 90)** combines defensive actions, duel efficiencies, PAdj events, and blocks.
    """)
    st.header("Physical + Technical Composites")
    st.markdown("""
**Work Rate/Intensity/Explosion** measure output normalized by distance (km), HI+running km, and explosive events.
    """)
    st.header("Radar Composite Metrics")
    st.markdown("""
Composites normalized to [0–100]: xG Buildup, Creativity, Progression, Defence, Involvement, Discipline, Finishing,
Poaching, Aerial Threat/Defence, Passing Quality, Box Threat.
    """ )
    st.stop()

if page == "Ferramenta de Busca":
    st.title("🔎 Ferramenta de Busca")
    st.caption("Filtre jogadores por **percentis mínimos** em um *position preset*. Percentis no dataset completo (inverte métricas negativas).")
    ...
    thresholds = {m: (global_min if use_global else st.slider(f"{m}", 0, 100, 70)) for m in metrics_for_preset}
    ...
    min_col = "Minutes played" if "Minutes played" in res.columns else ("Minutes" if "Minutes" in res.columns else None)
    if min_col is not None:
        res = res[pd.to_numeric(res[min_col], errors="coerce").fillna(0) >= float(min_minutes)]
    if team_filter and "Team" in res.columns:
        res = res[res["Team"].astype(str) == team_filter]
    if pos_filter and "Position" in res.columns:
    ...
    show_cols = [c for c in (base_cols + pct_col_names) if c in res.columns]
    st.markdown(f"**Jogadores encontrados:** {len(res)}")
    if show_cols:
        st.dataframe(res[show_cols].reset_index(drop=True))
        csv_bytes = res[show_cols].to_csv(index=False).encode("utf-8")
...
def _leaderboard(metric):
    ...
    if team_filter and "Team" in d.columns:
        d = d[d["Team"].astype(str) == team_filter]
    if pos_filter and "Position" in d.columns:
        d = d[d["Position"].astype(str).str.contains(pos_filter)]
    if metric not in d.columns:
        st.warning(f"Métrica {metric} não encontrada no dataset."); return
...
with colA:
    ...
    selected_presets = st.multiselect("Position presets (choose up to 3)", options=all_presets, default=[all_presets[0]])
    ...
    metrics_all = sorted([c for c in df_all.columns if c not in ID_COLS_CANDIDATES and pd.api.types.is_numeric_dtype(df_all[c])])
    default_metrics = metrics_from_presets[:16] if metrics_from_presets else []
    metrics_sel = st.multiselect("Metrics in radar (max 16)", options=metrics_all, default=default_metrics,
    ...
    if p1 and metrics_sel:
        ...
        pdf_buf = make_radar_bars_pdf_a4(
            df_all, p1, None if p2 == "—" else p2, metrics_sel, color_a, color_b,
            player_photo_bytes=player_photo_bytes, crest_bytes=crest_bytes,
        )
...
st.download_button("Download full CSV", df_all.to_csv(index=False).encode("utf-8"), file_name="composite_metrics_base.csv", mime="text/csv")
from __future__ import annotations
